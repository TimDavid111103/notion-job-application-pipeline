...
Repo layout for the aggregator-sourcer pipeline. Commands: [pipeline-commands.md](pipeline-commands.md).
...
| `cleanup-data.ts` | Wipe all temp `data/` files (`npm run cleanup:data`; keep scratch) |
...
| `cleanup.ts` | Delete everything under `data/` except `sourced-jobs.md` + `.gitkeep` |
...
`.cursor/skills/aggregator-sourcer/scripts/setup.sh`.
