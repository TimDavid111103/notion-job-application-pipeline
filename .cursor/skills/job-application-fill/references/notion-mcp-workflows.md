...
Single source of truth for MCP steps. Schema: [notion-tracker-schema.md](notion-tracker-schema.md).
Artifacts: [fill-data-formats.md](fill-data-formats.md).

Use `user-notion` MCP. Read tool schemas before each call.

## Step 0 — Schema preflight

`get_database` with database ID `32f1de14-69d8-803a-81ba-fb8cf47a1ccd`. Confirm **Status** option spellings match `scripts/lib/notion.ts`.
...
Read `data/url-health-results.json`. For each item with `status: "broken"` and `deletable: true`:
...
Helper: `statusUpdatePayload(status)` in `scripts/lib/notion.ts`.
