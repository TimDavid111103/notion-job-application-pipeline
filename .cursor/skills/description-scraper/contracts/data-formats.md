...
Single source of truth for runtime `data/scrape/` artifacts used by this skill.

Paths are defined in `scripts/lib/paths.ts`. Schemas and validators:
`scripts/lib/artifacts/scrape-artifacts.ts`. Cleanup: `npm run cleanup:data` removes all temporary
...
|---|---|---|---|
| `data/scrape/jobs-needing-descriptions.json` | Agent (MCP query) | Agent (step 4) | One run |
| `data/scrape/notion-scrape-queue.json` | Agent (step 4) | `scrape:descriptions` | One run |
| `data/scrape/scrape-results.json` | `scrape:descriptions` | Agent (steps 7–8) | One run |
...
## Jobs needing descriptions (`data/scrape/jobs-needing-descriptions.json`)
...
| Field | Rule |
|---|---|
| `database_id` | Must match `NOTION_DATABASE_ID` in `scripts/lib/notion/` |
...
MCP parsing: `parseTrackerRows()` in `scripts/lib/notion/`.

## Queue file (`data/scrape/notion-scrape-queue.json`)
...
| Field | Rule |
|---|---|
| `source_snapshot` | Always `"data/scrape/jobs-needing-descriptions.json"` |
...
## Results file (`data/scrape/scrape-results.json`)
...
```json
{
  ...
  "source_queue": "data/scrape/notion-scrape-queue.json",
  "summary": {
    "queued": 3,
    "ok": 2,
    "broken": 1,
...
| Field | Rule |
|---|---|
| `source_queue` | Always `"data/scrape/notion-scrape-queue.json"` |
...
Produced by `formatJobDescriptionMarkdown()` in `scripts/lib/scrape/scrape-markdown.ts`.
...
`isEmptyPageMarkdown(md)` in `scripts/lib/notion/`:
