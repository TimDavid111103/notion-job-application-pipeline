...
2. **Workday (`myworkdayjobs.com`)**: fetch via public CXS JSON API first
   (`scripts/lib/workday.ts`) — no browser needed. Falls back to Playwright on API miss.
...
9. **Language check** (`isEnglishDescription` in `scripts/lib/language.ts`): non-English
...
(`…/AccentureCareers/job/London/…`). Implementation: `scripts/lib/workday.ts`.
...
Applied during markdown formatting (`scripts/lib/scrape-markdown.ts`), generalized across
hosts:
...
same `.auth/handshake.json` session created by the **aggregator-sourcer** skill
(`npm run auth:handshake`) — `aggregatorForUrl()` in `scripts/lib/browser.ts` routes
