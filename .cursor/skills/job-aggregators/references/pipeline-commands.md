...
First-time from skill folder: `bash .cursor/skills/job-aggregators/scripts/setup.sh`.
...
npx tsx scripts/sources/jack-empty.ts   # Jack inbox + Saved clean-out
```

## Logging pipeline

```bash
npm run cleanup:data               # step 5 & 9 — temp artifacts only
...
```bash
npm run log:notion                 # payloads without Notion dedup — debug only
